...
void OPLSystem::allocateSeats() {
  ...
  int quota = totalSeats > 0 ? getVotes() / (totalSeats > 0 ? totalSeats : 1) : 0;
...
std::string OPLSystem::displayResults() {
  std::string result =
      "A OPL election was run to fill " + std::to_string(getSeats()) +" seats. "
      "There were " + std::to_string(getVotes()) + " votes. The results are below:\n\n";

  // We call our helper function to build the table of winning and losing candidates
  result += OPLCandidates(candidates, winners, parties, partySeats, getVotes());
...
std::string OPLSystem::auditResults() {
  ...
  result += "Total Votes: " + std::to_string(getVotes()) + "\n";
  ...
  for (size_t i = 0; i < parties.size(); i++) {
    ...
    quota = getSeats() > 0 ? getVotes() / (getSeats() > 0 ? getSeats() : 1) : 0;
    ...
    perOfVotes = static_cast<float>(parties.at(i).second) / static_cast<float>(getVotes()) * 100.0;
